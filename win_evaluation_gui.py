...
def calculate(hands, table_cards):
    ...
    win_tie_counts, amount_of_tables = poker_func.win_probability(hands, table_cards, available_cards, 50000)
    print(timeit.default_timer() - start)
    strings = []

    for i, win_tie in enumerate(win_tie_counts):
        strings.append(f"{hands[i][0]} {hands[i][1]}\n Win: {win_tie[0] / amount_of_tables * 100:.2f} %\nTie: {win_tie[1] / amount_of_tables * 100:.2f} %")
        print(f"{hands[i]},", end=" ")
...
def calculate_unknown(hero_hand, table_cards):
    ...
    start = timeit.default_timer()
    results = poker_func.win_probability_against_unknown_cards(hero_hand, table_cards, available_cards, num_of_cards_to_generate=1225, num_of_tables=100, choice='automatic')
    print(timeit.default_timer() - start)

    # noinspection PyListCreation
    strings = []

    strings.append(f"{hero_hand[0]} {hero_hand[1]}\n Win: {results[0] / results[3] * 100:.2f} %\nTie: {results[2] / results[3] * 100:.2f} %")
    ...
    if num_of_hands:
        ...
        for i, label in enumerate(labels):
            if show[i]:
                ...
                match i:
                    case 0:
                        labels[0].place(relx=x_row_2 + x_diff, rely=y_row_1 + y_diff, anchor='center')
                    case 1:
                        labels[1].place(relx=x_row_3 + x_diff, rely=y_row_2 + y_diff, anchor='center')
                    case 2:
                        labels[2].place(relx=x_row_3 + x_diff, rely=y_row_3 + y_diff, anchor='center')
                    case 3:
                        labels[3].place(relx=x_row_2 + x_diff, rely=y_row_4 + y_diff, anchor='center')
                    case 4:
                        labels[4].place(relx=x_row_1 + x_diff, rely=y_row_3 + y_diff, anchor='center')
                    case 5:
                        labels[5].place(relx=x_row_1 + x_diff, rely=y_row_2 + y_diff, anchor='center')
...
img = (Image.open("images\\table.png"))
...
table_label = tk.Label(frame, text="", bg="#f0f0f0", bd=5, fg="black", font=("Helvetica", 15))


for _ in range(6):
    text_boxes.append(tk.Text(frame,
                              height=1,
                              width=6),
                      )
...
labels = []
for _ in range(6):
    labels.append(tk.Label(frame, text="", bg="#f0f0f0", bd=5, fg="black", font=("Helvetica", 15)))
...
calculate_btn = tk.Button(frame,
                          text="Calculate",
                          command=get_hand_input, font=("Helvetica", 20))
